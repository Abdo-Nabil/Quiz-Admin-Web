class AppStrings {
  //---------------Shared preferences keys----------------------
  static const String storedLocale = 'storedLocale';
  static const String storedRoute = 'storedRoute';
  static const String isLightTheme = 'isLightTheme';
  static const String storedToken = 'storedToken';
  static const String storedId = 'storedId';

  //---------------On boarding---------------------------------
  static const String done = 'done';
  static const String title1 = 'title1';
  static const String description1 = 'description1';

  //---------------Authentication-----------------------------
  static const String email = 'email';
  static const String password = 'password';
  static const String quizAdmin = 'quizAdmin';
  static const String reenterPassword = 'reenterPassword';
  static const String name = 'name';
  //
  static const String login = 'login';
  static const String register = 'register';
  static const String signOut = "signOut";
  static const String registerNow = 'registerNow';
  static const String loginNow = 'loginNow';
  static const String dontHaveAccount = 'dontHaveAccount';
  static const String alreadyHaveAccount = 'alreadyHaveAccount';
  static const String twoPasswordsNotIdentical = 'twoPasswordsNotIdentical';
  //errors
  static const String weakPassword = 'weakPassword';
  static const String emailInUse = 'emailInUse';
  static const String userNotFound = 'userNotFound';
  static const String wrongPassword = 'wrongPassword';
  static const String internetConnectionError = 'internetConnectionError';
  static const String someThingWentWrong = 'someThingWentWrong';
  static const String emptyValue = 'emptyValue';
  static const String savingTokenError = 'savingTokenError';
  static const String invalidEmail = 'invalidEmail';
  static const String notFound = 'notFound';
  static const String passwordMustBe = 'passwordMustBe';
  static const String removingTokenError = "removingTokenError";
  //

  //---------------Home Screen---------------------------------

  static const String hello = 'hello';
  static const String availableQuizzes = 'availableQuizzes';
  static const String startQuizNow = 'startQuizNow';
  static const String minute = 'minute';
  static const String updateApp = 'updateApp';
  static const String update = 'update';
  static const String question = 'question';

  //---------------Routes---------------------------------
  static const String notFoundPage = "notFoundPage";

  //---------------Quiz screen---------------------------------
  static const String finish = 'finish';
  static const String finishQuiz = "finishQuiz";
  static const String timeOut = "timeOut";
  static const String quizTimeEnded = "quizTimeEnded";

  //---------------Score screen---------------------------------
  static const String toHomeScreen = "toHomeScreen";
  static const String score = "score";
  static const String youAreBlocked = 'youAreBlocked'; // 1.0.0+1
  static const String contact = 'contact'; // 1.0.0+1
  static const String canotLaunch = 'canotLaunch'; // 1.0.0+1
  static const String quizName = 'quizName'; // 1.0.0+1
  static const String quizDuration = 'quizDuration'; // 1.0.0+1
  static const String choices = 'choices'; // 1.0.0+1
  static const String correctAnswer = 'correctAnswer'; // 1.0.0+1
  static const String createQuiz = 'createQuiz'; // 1.0.0+1
  static const String fieldCantBeEmpty = 'fieldCantBeEmpty'; // 1.0.0+1
  static const String enterANumber = 'enterANumber'; // 1.0.0+1
  static const String enterPositiveNum = 'enterPositiveNum'; // 1.0.0+1
  static const String deleteQuiz = 'deleteQuiz'; // 1.0.0+1
  static const String editQuiz = 'editQuiz'; // 1.0.0+1
  static const String correctAnsMustBeOneOfChoices =
      'correctAnsMustBeOneOfChoices'; // 1.0.0+1

  //------------------------others-----------------------
  static const String appNameForRecentApp = "Quiz App";
  //
  static const String ok = 'ok';
  static const String cancel = "cancel";
  static const String retry = 'retry';
  static const String loading = 'loading';
  static const String alert = 'alert';
  static const String areYouSure = "areYouSure";
  //

  static const String appBuildNumber = 'appBuildNumber'; // 1.0.0+1
  static const String abdoEmail = 'abdo.sw.eng@gmail.com';
  static const String emailContact =
      'mailto:$abdoEmail?subject=Contact Request';
  static const String appUrlOnStore =
      "https://play.google.com/store/apps/details?id=com.AbdoNabil.quiz_app";
  static const String wordTranslationNotFound =
      '**** Exception **** Word Translation Not Found ****';
}
